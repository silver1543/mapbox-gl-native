...
std::vector<TileID> OfflineTilePyramidRegionDefinition::tileCover(SourceType type, uint16_t tileSize, const Tileset& tileset) const {
    double minZ = std::max<double>(coveringZoomLevel(minZoom, type, tileSize), tileset.minZoom);
    double maxZ = std::min<double>(coveringZoomLevel(maxZoom, type, tileSize), tileset.maxZoom);
