...
void Source::updateMatrices(const mat4 &projMatrix, const TransformState &transform) {
    for (const auto& pair : tiles) {
        ...
        transform.matrixFor(tile.matrix, tile.id, std::min(static_cast<uint8_t>(tileset->maxZoom), tile.id.z));
...
TileData::State Source::addTile(const TileID& tileID, const UpdateParameters& parameters) {
    ...
    if (!newTile->data) {
        ...
        if (type == SourceType::Raster) {
            ...
            if (type == SourceType::Vector) {
                monitor = std::make_unique<VectorTileMonitor>(normalizedID, parameters.pixelRatio, tileset->tiles.at(0), parameters.fileSource);
...
bool Source::findLoadedChildren(const TileID& tileID, int32_t maxCoveringZoom, std::vector<TileID>& retain) {
    bool complete = true;
    int32_t z = tileID.z;
    auto ids = tileID.children(tileset->maxZoom);
 ...
 */
void Source::findLoadedParent(const TileID& tileID, int32_t minCoveringZoom, std::vector<TileID>& retain, const style::UpdateParameters& parameters) {
    for (int32_t z = tileID.z - 1; z >= minCoveringZoom; --z) {
        const TileID parent_id = tileID.parent(z, tileset->maxZoom);
...
bool Source::update(const style::UpdateParameters& parameters) {
    ...
    int32_t minCoveringZoom = util::clamp<int32_t>(zoom - 10, tileset->minZoom, tileset->maxZoom);
    int32_t maxCoveringZoom = util::clamp<int32_t>(zoom + 1,  tileset->minZoom, tileset->maxZoom);

    if (zoom >= tileset->minZoom) {
