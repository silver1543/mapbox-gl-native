...
TEST(OfflineTilePyramidRegionDefinition, TileCoverEmpty) {
    ...
    auto result = region.tileCover(SourceType::Vector, 512, tileset);
...
TEST(OfflineTilePyramidRegionDefinition, TileCoverZoomIntersection) {
    ...
    tileset.minZoom = 0;
    auto resultIntersection = region.tileCover(SourceType::Vector, 512, tileset);
    ASSERT_EQ(1, resultIntersection.size());

    tileset.minZoom = 3;
    auto resultNoIntersection = region.tileCover(SourceType::Vector, 512, tileset);
...
TEST(OfflineTilePyramidRegionDefinition, TileCoverTileSize) {
    ...
    auto result512 = region.tileCover(SourceType::Vector, 512, tileset);
    ASSERT_EQ(1, result512.size());
    ASSERT_EQ(0, result512[0].z);

    auto result256 = region.tileCover(SourceType::Vector, 256, tileset);
...
TEST(OfflineTilePyramidRegionDefinition, TileCoverZoomRounding) {
    ...
    auto resultVector = region.tileCover(SourceType::Vector, 512, tileset);
    ASSERT_EQ(1, resultVector.size());
    ASSERT_EQ(0, resultVector[0].z);

    auto resultRaster = region.tileCover(SourceType::Raster, 512, tileset);
